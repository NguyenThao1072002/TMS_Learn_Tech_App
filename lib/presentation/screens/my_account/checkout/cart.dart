...
class _CartScreenState extends State<CartScreen> {
  ...
  Future<void> _selectPreselectedItem() async {
    ...
    for (var item in items) {
      ...
      if (isMatch) {
        ...
        // Nếu là combo thì tải chi tiết combo
        else if (item.type.toUpperCase() == 'COMBO' && item.courseBundleId != null) {
  ...
  Future<void> _toggleSelectAll(bool? value) async {
    ...
    final combos = items.where((item) => item.type.toUpperCase() == 'COMBO').toList();
    final courses = items.where((item) => item.type.toUpperCase() == 'COURSE').toList();
    final others = items.where((item) => 
        item.type.toUpperCase() != 'COMBO' && 
        item.type.toUpperCase() != 'COURSE').toList();
    ...
    if (_selectedComboDetail != null) {
      coursesInSelectedCombos = _selectedComboDetail!.courses.map((c) => c.id).toSet();
    }
    
    // Nếu có nhiều combo đã chọn, cần tải thông tin chi tiết của từng combo
    for (var combo in combos) {
      if (combo.courseBundleId != null && _selectedItems[combo.cartItemId] == true) {
        try {
          final comboDetail = await _courseController.getComboDetail(combo.courseBundleId!);
          if (comboDetail != null) {
            coursesInSelectedCombos.addAll(comboDetail.courses.map((c) => c.id));
          }
        } catch (e) {
    ...
    for (var course in courses) {
      if (course.courseId != null && !coursesInSelectedCombos.contains(course.courseId)) {
        setState(() {
                      ...
                      child: ListView(
                        children: [
                          ...
                          if (_hasSelectedItems) ...[
                            ...
                            if (_selectedComboDetail != null) _buildComboDetailSection(),
  ...
  Future<void> _loadComboDetail(int comboId) async {
    ...
    try {
      ...
      if (comboDetail != null) {
        print('Đã tải chi tiết combo: ${comboDetail.name} với ${comboDetail.courses.length} khóa học');
      } else {
  ...
  Future<void> _toggleItemSelection(CartItem item, bool isSelected) async {
    ...
    if (isSelected) {
      ...
      if (item.type.toUpperCase() == 'COURSE' && item.courseId != null) {
        final availableCombos = await _findCombosContainingCourse(item.courseId!);
      ...
      if (isSelected && item.type.toUpperCase() == 'COMBO' && item.courseBundleId != null) {
    ...
    setState(() {
      ...
      // Nếu bỏ chọn, xóa thông tin chi tiết combo nếu đang hiển thị của item đó
      if (!isSelected && item.type.toUpperCase() == 'COMBO' && 
          _selectedComboDetail != null && 
    ...
    if (isSelected) {
      // Nếu là khóa học, tải danh sách combo có chứa khóa học đó
      if (item.type.toUpperCase() == 'COURSE' && item.courseId != null) {
    ...
    // Kiểm tra nếu không còn khóa học nào được chọn
          ...
          Padding(
            ...
            child: Center(
              child: ElevatedButton.icon(
                onPressed: () => _navigateToComboDetail(_selectedComboDetail!.id),
                style: ElevatedButton.styleFrom(
                  foregroundColor: Colors.white,
                  backgroundColor: Colors.purple,
  ...
  bool _isCourseInBundle(int courseId, int bundleId) {
    if (_selectedComboDetail != null && _selectedComboDetail!.id == bundleId) {
      return _selectedComboDetail!.courses.any((course) => course.id == courseId);
    }
    return false;
  }
  ...
  bool _hasConflict(CartItem newItem, bool isSelected) {
    ...
    if (newItem.type.toUpperCase() == 'COMBO' && newItem.courseBundleId != null) {
  ...
  Future<List<CartItem>> _findCombosContainingCourse(int courseId) async {
    ...
    final combos = _cartController.cartItems.value
        .where((item) => item.type.toUpperCase() == 'COMBO' && item.courseBundleId != null)
        .toList();
    ...
    for (var combo in combos) {
      ...
      if (_selectedComboDetail == null || _selectedComboDetail!.id != combo.courseBundleId) {
        try {
          final comboDetail = await _courseController.getComboDetail(combo.courseBundleId!);
          if (comboDetail != null && comboDetail.courses.any((course) => course.id == courseId)) {
            result.add(combo);
          }
        } catch (e) {
        ...
        if (_selectedComboDetail!.courses.any((course) => course.id == courseId)) {
          result.add(combo);
        }
      }
    }
  ...
  void _showSuggestComboDialog(CartItem courseItem, List<CartItem> availableCombos) {
    showDialog(
      context: context,
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      bool isComboSaving = courseItem.price > 0 && 
                                           ((combo.price / courseItem.price) < 1.5); // Combo tiết kiệm nếu giá < 1.5 lần giá khóa học
                            ...
                            ListTile(
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  ...
                                  if (combo.courseBundleId != null)
                                    FutureBuilder(
                                      future: _courseController.getComboDetail(combo.courseBundleId!),
                                      builder: (context, snapshot) {
                                        if (snapshot.hasData && snapshot.data != null) {
                                          final comboDetail = snapshot.data!;
                                          final courseCount = comboDetail.courses.length;
                                          final savings = comboDetail.getSavings();
                                ...
                                children: [
                                  OutlinedButton(
                                    onPressed: () {
                                      ...
                                      if (combo.courseBundleId != null) {
                                        _navigateToComboDetail(combo.courseBundleId!);
                                      }
                                    },
                                  ...
                                  ElevatedButton(
                                    onPressed: () {
                                      ...
                                      setState(() {
                                        _selectedItems[courseItem.cartItemId] = false; // Bỏ chọn khóa học
                                        _selectedItems[combo.cartItemId] = true; // Chọn combo
                                      });
                                      // Tải chi tiết combo
                                      if (combo.courseBundleId != null) {
